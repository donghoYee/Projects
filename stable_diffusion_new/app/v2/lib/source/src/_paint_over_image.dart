...
class ImagePainterState extends State<ImagePainter> {
  ...
  Future<ui.Image> _renderMask() async {
    final img = await rootBundle.load("assets/white.png");
    final codec = await ui.instantiateImageCodec(
      img.buffer.asUint8List(),
      targetHeight: _image!.height.toInt(),
      targetWidth: _image!.width.toInt(),
    );
    final _image_white = (await codec.getNextFrame()).image;

    //final _image_white = await _convertImage(Uint8List.view(img.buffer));
    final recorder = ui.PictureRecorder();
    final canvas = Canvas(recorder);
    final painter = DrawImage(image: _image_white, paintHistory: _paintHistory);
    final size = Size(_image!.width.toDouble(), _image!.height.toDouble());
    painter.paint(canvas, size);
    return recorder
        .endRecording()
  ...
  PopupMenuItem _showColorPicker(Controller controller) {    /////////this picks the colors
    return PopupMenuItem(
        enabled: false,
        child: Center(
          child: Wrap(
  ...
  PopupMenuItem _showColorPicker_new(Controller controller) {    /////////this picks the colors
    return PopupMenuItem(
        enabled: false,
        child: Center(
          child: HueRingPicker(
  ...
  Future<Uint8List?> exportMask() async{
    late ui.Image _convertedImage;
    _convertedImage = await _renderMask();
    final byteData =
    await _convertedImage.toByteData(format: ui.ImageByteFormat.png);
    return byteData?.buffer.asUint8List();
  }
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          widget.initialStrokeWidth! < 20.0 ?
          Padding(
            ...
            child: ValueListenableBuilder<Controller>(
                ...
                builder: (_, controller, __) {
                  return PopupMenuButton(
                    ...
                    //itemBuilder: (_) => [_showColorPicker_new(controller)],
                  );
                }),
          ):SizedBox(),
            ...
            child: ValueListenableBuilder<Controller>(
                ...
                builder: (_, controller, __) {
                  return PopupMenuButton(
                    ...
                    icon: widget.colorIcon ??
                        Container(
                          padding: const EdgeInsets.fromLTRB(0, 2, 0, 2.0),
                          child: Center(
                            child: Text(
                              "More \nColors",
                              style: TextStyle(
          ...
          IconButton(
              tooltip: textDelegate.undo,
              icon:
                  Icon(Icons.undo_sharp),//widget.undoIcon ?? Icon(Icons.reply, color: Colors.grey[700]),
              onPressed: () {
